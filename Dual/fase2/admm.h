...
public:
    ...
    float get_physical_u(int idx) const {
        if (idx == my_idx) return u_own[my_idx]; // O meu próprio duty cycle
        return network_u[idx][idx];              // O duty cycle que o vizinho assumiu
    }
};
